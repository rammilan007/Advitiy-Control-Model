import numpy as np
from constants_1U import RESISTANCE, INDUCTANCE, PWM_AMPLITUDE, PWM_FREQUENCY, CONTROL_STEP,n
import math
...
def getCurrentList(v_duty_cycle):
    '''
    ...
    '''
    T = 1/PWM_FREQUENCY
    N = int(2*n*CONTROL_STEP/T) #number of sampling points in a control step
    N_T=int(CONTROL_STEP/T)  # number of time periods in a control step
    # forming time array according to variable step size
    h_ON = min(v_duty_cycle*T/n)
    h_OFF = min((1-v_duty_cycle)*T/n)
    t=np.array([])
    for c in range(0,N_T):
       t_ON=np.linspace(0, min(v_duty_cycle*T),n)
       t_OFF=np.linspace(min(v_duty_cycle*T), T, n)
       t_T=np.append(t_ON,t_OFF)
       t=np.append(t,t_T+c*T)
...
# =============================================================================
        for k in range(3):
            ...
            if (t1 > 0.0 and leq(t1,h_OFF)) or (t2 > 0.0 and leq(t2,h_ON)): 
                v_t_prev[k] = t[i-1]
